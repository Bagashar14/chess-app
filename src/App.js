...
export default function App() {
  ...
  function onDrop(sourceSquare, targetSquare) {
    ...
    if (!newGame.isGameOver()) {
      setTimeout(() => makeStockfishMove(newGame), 300);
    }
    return true;
  }
  ...
  async function getCoachAnalysis() {
    if (history.length < 2) {
      setCoachMessage("Сыграйте несколько ходов, чтобы получить анализ!");
      return;
    }
    ...
    try {
      const response = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        body: JSON.stringify({
          ...
          messages: [
            {
              role: "user",
              content: `Ты шахматный тренер. Проанализируй эту партию и дай короткий совет на русском языке (3-4 предложения). Укажи одну главную ошибку и один хороший ход. Ходы: ${moves}`,
            },
          ],
        }),
      });
      ...
      setCoachMessage("Ошибка при получении анализа. Попробуйте снова.");
    }
          ...
          <div style={{ background: isDark ? "#1e1e2e" : "#fff", borderRadius: 12, padding: 16 }}>
            ...
            <button
              ...
              style={{
                ...
                borderRadius: 8,
                border: "none",
                background: loadingCoach ? "#555" : "#10b981",
                color: "#fff",
                fontWeight: 700,
                cursor: loadingCoach ? "not-allowed" : "pointer",
                fontSize: 14,
              }}
            >
            ...
            {coachMessage && (
              <div
                style={{
                  marginTop: 12,
                  padding: 12,
                  background: isDark ? "#0f2d1f" : "#ecfdf5",
                  borderRadius: 8,
                  fontSize: 13,
                  lineHeight: 1.6,
